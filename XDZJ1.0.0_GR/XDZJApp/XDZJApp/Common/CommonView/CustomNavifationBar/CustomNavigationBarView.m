...
-(void)layoutTitleViewWith:(NSString *)title andColorStyle:(NavigationBarViewBackColor)colorStyle{
    ...
    int iTitleTopY = NavHeight - 39;
    if (IS_IPHONE_X_MORE)
     {
        iTitleTopY = NavHeight - 43;
     }
    
    _titleLab = [[UILabel alloc] initWithFrame:CGRectMake(60.f, iTitleTopY, SCREEN_WIDTH - 60.f * 2, 35.f)];
...
//    _titleLab.font = [UIFont fontWithName:@"Helvetica-Bold" size:16];
    ...
    if (colorStyle == NavigationBarViewBackColorWhite) {
        ...
        UIImageView *backdropImg = [[UIImageView alloc]initWithFrame:self.frame];
        [self addSubview:backdropImg];
        backdropImg.image = [UIImage imageNamed:@"naviBackdrop"];
        backdropImg.userInteractionEnabled = YES;
    ...
    }else if(colorStyle == NavigationBarViewBackColorBlue){
        //蓝底白字
        _titleLab.textColor = [UIColor whiteColor];
        self.backgroundColor = UIColorFromRGB(0x3f95fd);
    }
