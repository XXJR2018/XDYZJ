...
-(void)setPlaceHolder:(NSString *)placeHolder{
    if (placeHolder) {
        ...
        if (_colorStyle) {
//            _colorStyle = NO;
            _holderLabel.textColor = UIColorFromRGB(0x3585e6);
        }
    }
}
...
-(void)layoutHolderLabel:(NSString *)placeHolder{
    _holderLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.width - 230.0, (CellHeight44 - CellTitleFontSize)/2, 200.0, CellTitleFontSize)];
...
-(void)layoutImagePicker:(BOOL)picker{
    ...
    if (picker) {
        ...
        _rightImage.frame = CGRectMake(self.width - 20, (CellHeight44 - 11)/2, 7, 11);
    }
...
-(UIView *)itemsView{
    ...
    bgView.backgroundColor = UIColorFromRGBA(0xffffff, 0.9);
...
// 弹出框选择
-(PickerView *)initWithTypeTitle:(NSString *)title placeHolder:(NSString *)placeHolder itemArray:(NSArray *)items origin_Y:(CGFloat)origin_Y origin_X:(CGFloat)origin_X{
...
// 跳页面选择
-(PickerView *)initWithTypeTitle:(NSString *)title placeHolder:(NSString *)placeHolder origin_Y:(CGFloat)origin_Y{
    self = [super initWithFrame:CGRectMake(0, origin_Y, SCREEN_WIDTH, CellHeight44)];
    if (self) {
        self.linetype = LineTypeBotton;
        _colorStyle = YES;
