...
@interface XHDatePickerView_1 ()<UIPickerViewDelegate,UIPickerViewDataSource,UIGestureRecognizerDelegate> {
    //日期存储数组
...
@implementation XHDatePickerView_1
...
-(void)setupUI {
    ...
    //self.buttomView.backgroundColor = [UIColor blueColor];
    ...
    UIButton  *btnCancel = [[UIButton alloc] initWithFrame:CGRectMake(0, self.buttomView.height-40, 150, 40)];
    [self.buttomView addSubview:btnCancel];
    btnCancel.backgroundColor = [ResourceManager viewBackgroundColor];
    [btnCancel setTitle:@"取消" forState:UIControlStateNormal];
    [btnCancel setTitleColor:[ResourceManager lightGrayColor] forState:UIControlStateNormal];
...
-(void)defaultConfig {
    ...
    if (!self.maxLimitDate) {
        self.maxLimitDate = [NSDate date:@"2049-12-31" WithFormat:@"yyyy-MM-dd"];
    }
    //最小限制
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    switch (self.dateType) {
        case DateTypeStartDate:
        {
            ...
            NSString *string = [format stringFromDate:_startDate];
            _beginTimeLabel.text = string;
        ...
        default:
        {
            ...
            NSDateFormatter *format = [[NSDateFormatter alloc] init];
            format.dateFormat = @"yyyy-MM-dd";
            NSString *string = [format stringFromDate:_endDate];
            _endTimeLabel.text = string;
            break;
        }
    }
    ...
    if(self.dateType == DateTypeEndDate)
     {
        ...
        NSDate *date = [dateFormatter dateFromString:_endTimeLabel.text];
        [self getNowDate:date animated:NO];
     }
    else
     {
        NSDate *date = [dateFormatter dateFromString:_beginTimeLabel.text];
        [self getNowDate:date animated:NO];
     }
       ...
       if (0 ==iFirst)
        {
           ...
           [self getNowDate:dateNow animated:NO];
           iFirst = 1;
        }
    }*/
}
...
- (IBAction)doneAction:(UIButton *)btn {
    ...
    //NSString *strDate = [_dateFormatter stringFromatDate:_startDate];
    

    
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd"];
    _startDate = [dateFormatter dateFromString:_beginTimeLabel.text];
    _startDate = [[NSDate date] initWithTimeInterval:8 *60 * 60 sinceDate:_startDate];
    _endDate = [dateFormatter dateFromString:_endTimeLabel.text];
    _endDate = [[NSDate date] initWithTimeInterval:8 *60 * 60 sinceDate:_endDate];
    
    if ([_endDate timeIntervalSinceDate:_startDate] < 0.0)
     {
        [MBProgressHUD showErrorWithStatus:@"开始时间不能大于结束时间" toView:self];
        return;
     }
...
- (void)getNowDate:(NSDate *)date animated:(BOOL)animated
{
    ...
    NSArray *indexArray;
    self.datePickerStyle = DateStyleShowYearMonthDay; // 此处写死
