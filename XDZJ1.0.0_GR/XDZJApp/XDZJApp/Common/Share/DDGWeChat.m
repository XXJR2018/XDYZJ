...
-(id)init
{
    ...
    if(self = [super init]){
        //向微信注册
       ret  = [WXApi registerApp:APPID_WC];
    }
    return self;
}
...
-(BOOL) shareIMG:(NSDictionary *)items shareScene:(int)scene{
    ...
    UIImage* img = [UIImage imageWithData:[items objectForKey:@"image"]];
   
    // 发送的原图， 分享后，分享的人可以看到
    imageObject.imageData = UIImageJPEGRepresentation(img, 0.3);
    message.mediaObject = imageObject;
    
    // 发送的压缩图片，在分享前，用户自己可以看到
    UIImage* imgYS = [UIImage imageWithData:[self imageWithImage:img scaledToSize:CGSizeMake(200, 400)]];
    [message setThumbImage:imgYS];
...
-(void) onResp:(BaseResp*)resp
{
    ...
    if (resp)
     {
        NSLog(@"resp :%@  resp.errCode:%d, resp.errStr:%@" ,resp, resp.errCode, resp.errStr);
