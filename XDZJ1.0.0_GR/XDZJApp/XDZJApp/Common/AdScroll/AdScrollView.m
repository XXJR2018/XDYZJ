...
NSUInteger currentImage = 0;//记录中间图片的下标,开始总是为1
...

{
    ...
    //当前Banner数量   当 _currentBannerNum ==1 时，取消滚动（ps：默认还是三张，填充在中间）
    int _currentBannerNum;
}
...
-(void)tapClick{
    if (_currentBannerNum < 1)
        return;
    
    if (self.postADViewControllerBlock) {
        self.postADViewControllerBlock(self.urlArray[currentImage]? self.urlArray[currentImage]: @"http://www.duoduozhifu.com",self.adTitleArray[currentImage]? self.adTitleArray[currentImage]: @"多多智富");
    }
}
...
- (void)setImageNameArray:(NSArray *)imageNameArray
{
    ...
    {
        _currentBannerNum = 0;
        _imageNameArray = @[@"home_baner_1", @"home_baner_1"];
    }
    else if (_imageNameArray.count == 1)
    {
        _currentBannerNum = 1;
        self.scrollEnabled = NO;
       _imageNameArray = @[@"home_baner_1",_imageNameArray[0], @"home_baner_1"];
    
    }
    else if (_imageNameArray.count > 1)
    {
        _currentBannerNum = (int)_imageNameArray.count;
    }

    
    if (_currentBannerNum < 1)
    {
       _leftImageView.image =  [UIImage imageNamed:@"home_baner_1"];//[UIImage imageNamed:@"banner_placeholderImage_1"];
       _centerImageView.image = [UIImage imageNamed:@"home_baner_1"];//[UIImage imageNamed:_imageNameArray[0]];
       _rightImageView.image = [UIImage imageNamed:@"home_baner_1"];//[UIImage imageNamed:_imageNameArray[1]];

        
    }
    else  if(_currentBannerNum == 1)
    {
        _leftImageView.image = [UIImage imageNamed:_imageNameArray[0]];
        [_centerImageView sd_setImageWithURL:[NSURL URLWithString:_imageNameArray[1]] placeholderImage:[UIImage imageNamed:@"home_baner_1"] options:SDWebImageProgressiveDownload];
        _rightImageView.image = [UIImage imageNamed:_imageNameArray[2]];
    }
    else
    {
        [_leftImageView sd_setImageWithURL:[NSURL URLWithString:_imageNameArray[_imageNameArray.count - 1]] placeholderImage:[UIImage imageNamed:@"home_baner_1"] options:SDWebImageProgressiveDownload];
        [_centerImageView sd_setImageWithURL:[NSURL URLWithString:_imageNameArray[0]] placeholderImage:[UIImage imageNamed:@"home_baner_1"] options:SDWebImageProgressiveDownload];
        [_rightImageView sd_setImageWithURL:[NSURL URLWithString:_imageNameArray[1]] placeholderImage:[UIImage imageNamed:@"home_baner_1"] options:SDWebImageProgressiveDownload];
    }
...
-(void)setUrlArray:(NSArray *)urlArray
{
    ...
    if (_urlArray.count < 1) {
        _urlArray = @[@"http://www.baidu.com", @"http://www.baidu.com"];
    }
...
-(void)setAdTitleArray:(NSArray *)adTitleArray
{
    ...
    if (_adTitleArray.count < 1) {
        _adTitleArray = @[@"袋袋金", @"袋袋金"];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    else
    {
       ...
       if(_bDefalutImage)
        {
           //不加载后台图片，全部为默认图片
           _imageNameArray = @[@"home_baner_1", @"home_baner_2"];
           _leftImageView.image = [UIImage imageNamed:_imageNameArray[(currentImage-1)%_imageNameArray.count]];
           _centerImageView.image = [UIImage imageNamed:_imageNameArray[currentImage%_imageNameArray.count]];
           _rightImageView.image = [UIImage imageNamed:_imageNameArray[(currentImage + 1)%_imageNameArray.count]];
        ...
        {
           [_leftImageView sd_setImageWithURL:[NSURL URLWithString:_imageNameArray[(currentImage-1)%_imageNameArray.count]] placeholderImage:[UIImage imageNamed:@"home_baner_1"] options:SDWebImageProgressiveDownload];
           [_centerImageView sd_setImageWithURL:[NSURL URLWithString:_imageNameArray[currentImage%_imageNameArray.count]] placeholderImage:[UIImage imageNamed:@"home_baner_1"] options:SDWebImageProgressiveDownload];
           [_rightImageView sd_setImageWithURL:[NSURL URLWithString:_imageNameArray[(currentImage + 1)%_imageNameArray.count]] placeholderImage:[UIImage imageNamed:@"home_baner_1"] options:SDWebImageProgressiveDownload];
        }
