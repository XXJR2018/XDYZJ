...
-(void)clickOKButton:(UIButton *)button{
    ...
    if(_label1_text.text.length <4)
     {
        [MBProgressHUD showErrorWithStatus:@"请输入验证码" toView:self];
        return;
     }
    
    params[@"telephone"] = _strPhone? _strPhone:@"18688952105";
    params[@"page"] = _strTime;//@"kjLogin";
...
-(void)setUp{
    ...
    _imgUrl = [NSString stringWithFormat:@"%@%@?page=%@", [PDAPI getBaseUrlString], @"xxcust/smsAction/imageCode",_strTime];  //  @"";http://192.168.10.192:8080/xxcust/smsAction/imageCode
