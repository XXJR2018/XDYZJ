...
-(void)handleData:(DDGAFHTTPRequestOperation *)operation{
    ...
    if (operation.tag == 10210) {
        ...
        NSDictionary *dic = dataDicionary[@"borrowInfo"];
        
        myLock =[dic[@"myLock"] boolValue];
        
        isLock = [dic[@"isLock"] boolValue];
        ...
        if (1 == status)
         {
            ...
            [self realQD];
                 ...
                 [curBorrowId isEqualToString:borrorId])
         {
            [self realQD];
         }
        else if (2 == status &&
...
-(void)layoutUI
{
    ...
    if (isLock &&
        !myLock)
     {
        [btnApply setTitle:@"已被锁定" forState:UIControlStateNormal];
        //btnApply.userInteractionEnabled = NO;
        btnApply.backgroundColor = [ResourceManager lightGrayColor];
        [btnApply setTitleColor:[ResourceManager mainColor] forState:UIControlStateNormal];
...
{
    // 设置头部背景 begin
    UIView *viewBG = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 150)];
    [scView addSubview:viewBG];
    viewBG.backgroundColor = [UIColor whiteColor];
    // 设置头部背景 end
    
    // 设置头部  begin
    viewHead =  [[UIView alloc] initWithFrame:CGRectMake(10, 0, SCREEN_WIDTH - 20, 150)];
    ...
    if (isLock)
     {
        UIImageView *imgSuo = [[UIImageView alloc] initWithFrame:CGRectMake(iCurentX, iTopY + 23, 10, 13)];
        [viewHead2 addSubview:imgSuo];
        //imgSuo.backgroundColor = [UIColor blueColor];
        imgSuo.image = [UIImage imageNamed:@"light_suo2"];
        
        UILabel *labelSDSJ = [[UILabel alloc] initWithFrame:CGRectMake(iCurentX + 15, iTopY+20, 150, 20)];
        ...
        labelSDSJ.font = [UIFont systemFontOfSize:13];
        labelSDSJ.textColor = UIColorFromRGB(0x666666);
        //labelSqshijian.backgroundColor = [UIColor blueColor];
        [viewHead2 addSubview:labelSDSJ];
     }
    else
     {
        labelSqshijian.top = iTopY + 10;
     }
    ...
    if (isLock &&
        !myLock)
     {
        [btnSuo setTitle:@"已被锁定" forState:UIControlStateNormal];
        //btnSuo.userInteractionEnabled = NO;
        btnSuo.backgroundColor = [ResourceManager lightGrayColor];
        [btnSuo setTitleColor:[ResourceManager mainColor] forState:UIControlStateNormal];
     }
    
    iTopY += btnSuo.height +25;
...
-(void) popQDMessage1:(NSDictionary*) dicData
{
    ...
    [alertView addButton:@"去解锁" red:YES actionBlock:^{
        ...
        VC.dataDic = dicData;
        VC.arrCustTickets = _arrCustTickets;
...
-(void)lightingOrderForCellAtIndex{
    ...
    NSString *api = ![PDAPI isTestUser] ? [NSString stringWithFormat:@"%@mjb/account/dai/rob/robFree",[PDAPI getBaseUrlString]] : [NSString stringWithFormat:@"%@mjb/account/dai/rob/scoreBorrow",[PDAPI getBaseUrlString]];
