...
{
    [super viewDidLoad];
    [(SIAlertView *)self.alertView setup];
}

#if __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_8_0
- (void)viewWillTransitionToSize:(CGSize)size withTransitionCoordinator:(id <UIViewControllerTransitionCoordinator>)coordinator{
    [(SIAlertView *)self.alertView resetTransition];
    [(SIAlertView *)self.alertView invaliadateLayout];
}
#else
...
- (void)show
{
    ...
    [self transitionInCompletion:^{
        ...
        // 添加点击消失的手势
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(dismissAnimated:)];
...
{
    [self dismissAnimated:YES cleanup:YES];
}
...
- (UIButton *)buttonForItemIndex:(NSUInteger)index
{
	...
	button.tag = index;
    button.backgroundColor = [UIColor colorWithRed:242.0/255.0 green:243.0/255.0 blue:244.0/255.0 alpha:1.0];
...
//			highlightedImage = [UIImage imageNamed:@"SIAlertView.bundle/button-default-d"];
			...
			break;
        case SIAlertViewButtonTypeRedWhite:
            [button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            [button setTitleColor:[UIColor whiteColor] forState:UIControlStateHighlighted];
            button.backgroundColor = [ResourceManager redColor2];
            break;
        case SIAlertViewButtonTypeRedGrey:
        default:
            [button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            [button setTitleColor:[UIColor whiteColor] forState:UIControlStateHighlighted];
            button.backgroundColor = [ResourceManager color_5];
            break;
	}
