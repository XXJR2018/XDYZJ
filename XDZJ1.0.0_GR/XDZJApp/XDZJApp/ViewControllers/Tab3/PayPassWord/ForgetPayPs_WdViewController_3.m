//
//  ForgetPayPs_WdViewController_3.m
//  XXJR
//
...
@interface ForgetPayPs_WdViewController_3 ()<UITextFieldDelegate>
{
    NSInteger _psWdType;
    NSString *_newPsWd_1;
    NSString *_newPsWd_2;
...
@implementation ForgetPayPs_WdViewController_3

- (void)viewDidLoad {
    [super viewDidLoad];
    [self layoutNaviBarViewWithTitle:@"重置支付密码"];
    self.view.backgroundColor = [ResourceManager viewBackgroundColor];
    
    [self.view addSubview:self.promptLabel];
    self.promptLabel.text = @"请重置支付密码，用于支付验证";
    ...
    DDGAFHTTPRequestOperation *operation = [[DDGAFHTTPRequestOperation alloc] initWithURL:[NSString stringWithFormat:@"%@%@",[PDAPI getBaseUrlString],@"xxcust/account/info/changeTradePwd"]
                                                                               parameters:parame HTTPCookies:[DDGAccountManager sharedManager].sessionCookiesArray
                                                                                  success:^(DDGAFHTTPRequestOperation *operation, id responseObject){
                                                                                      ...
                                                                                      [MBProgressHUD showSuccessWithStatus:@"支付密码重置成功" toView:self.view];
                                                                                      [self performBlock:^{
