...
{
    //删除数据
    [[DDGSetting sharedSettings]setSignId:nil];
    NSString *strReturn = nil;
    [NSKeyedArchiver archiveRootObject:strReturn toFile:AccountFile];
...
-(BOOL)isInfoFinished{
    if ([self.user.mobile isKindOfClass:[NSNull class]] || self.user.mobile == nil || self.user.mobile.length < 11) {
        ...
        for (NSString *key in [DDGAccountManager sharedManager].userInfo.allKeys) {
            NSString *value = [NSString stringWithFormat:@"%@",[[DDGAccountManager sharedManager].userInfo objectForKey:key]];
    ...
    if ([[DDGAccountManager sharedManager] isLoggedIn] && _sharedManager.userInfo == nil) {
        DDGAFHTTPRequestOperation *operation = [[DDGAFHTTPRequestOperation alloc] initWithURL:[PDAPI getUserBaseInfoAPI]
                                            ...
                                            success:^(DDGAFHTTPRequestOperation *operation, id responseObject){
                                                ...
                                                // 防止崩溃， 判断数组是否越界
                                                if ([operation.jsonResult.rows count] >= 1)
                                                 {
