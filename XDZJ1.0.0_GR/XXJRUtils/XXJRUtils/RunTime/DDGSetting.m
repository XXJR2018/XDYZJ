...
@synthesize currentAccounType = _currentAccounType;
...
{
    NSString* strTemp = [NSString stringWithFormat:@"%@", uid];
    [[NSUserDefaults standardUserDefaults] setObject:strTemp
...
- (NSString *)signId
{
    NSString *signId = [[NSUserDefaults standardUserDefaults]
                        objectForKey:kCurrentSignIdKey];
    
    return signId ? signId : @"0";
}

- (void)setSignId:(NSString *)signId
{
    [[NSUserDefaults standardUserDefaults] setObject:signId
                                              forKey:kCurrentSignIdKey];
}

-(int)currentAccounType{
    return  1; // 写死为信贷经理
    
//    if (_currentAccounType) {
//        return _currentAccounType;
//    }
//    
//    @synchronized(self){
//        NSNumber *currentAccounType = [[NSUserDefaults standardUserDefaults]
//                                       objectForKey:kCurrentAccounTypeKey];
//        _currentAccounType = currentAccounType.intValue;
//    }
//    
//    return _currentAccounType ?: 0;
}

-(void)setCurrentAccounType:(int)currentAccounType{
    _currentAccounType = currentAccounType;
    
    [[NSUserDefaults standardUserDefaults] setObject:@(currentAccounType)
                                              forKey:kCurrentAccounTypeKey];
}
