...
{
    DDGASSERT(NO);
    NSLog(@"字典调用dic[0]形式，索引:%lu, 数据:%@", (unsigned long)idx, self);
    return nil;
}
...
{
    DDGASSERT(NO);
    NSLog(@"字典调用dic[0]=x形式，索引:%lu, 数据:%@", (unsigned long)index, self);
}

- (id)objectAtIndex:(NSUInteger)index
{
    DDGASSERT(NO);
    NSLog(@"字典调用数组方法objectAtIndex:%ld， 数据:%@", (unsigned long)index, self);
    return nil;
}
...
{
    DDGASSERT(NO);
    NSLog(@"字符串调用dic[0]形式，索引:%lu, 数据:%@", (unsigned long)idx, self);
    return nil;
}
...
{
    DDGASSERT(NO);
    NSLog(@"字符串调用dic[0]=x形式，索引:%lu, 数据:%@", (unsigned long)index, self);
}

- (id)objectAtIndex:(NSUInteger)index
{
    DDGASSERT(NO);
    NSLog(@"字符串调用数组方法objectAtIndex:%ld， 数据:%@", (unsigned long)index, self);
    return nil;
}
