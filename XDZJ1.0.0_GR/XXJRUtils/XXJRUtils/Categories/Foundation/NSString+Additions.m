...
- (BOOL)isMobileNumber
{
    return [self matches:@"^(1(([1-9][0-9])|(47)|[8][0123456789]))\\d{8}$"];
}
...
{
    return self.length >= 6 && self.length <= 12 && ![self containsString:@" "];
}
...
- (NSString *)stringByReplacingRegularString:(NSString *)regString withString:(NSString *)replaceString
{
    ...
    if (error)
    {
        //        DDGVerboseLog(@"regular expression error: %@", [error localizedDescription]);
        return self;
    }
...
{
    return [self judgePassWordLegal:self];
}
...
-(BOOL) judgePassWordLegal:(NSString *)pass
{
    ...
    if ([pass length] >= 6){
        ...
        NSString * regex =  @"^(?=.*?[a-zA-Z])(?=.*?[0-9])[\\s\\S]{6,12}$";
