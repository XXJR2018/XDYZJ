...
- (void)layoutSubviews {
	...
	CGSize labelSize = MB_MULTILINE_TEXTSIZE(label.text, label.font, CGSizeMake(maxWidth,200), label.lineBreakMode);
...
- (UIImage *)applyBlurWithCrop:(CGRect) bounds resize:(CGSize) size blurRadius:(CGFloat) blurRadius tintColor:(UIColor *) tintColor saturationDeltaFactor:(CGFloat) saturationDeltaFactor maskImage:(UIImage *) maskImage {
    ...
    if (hasBlur || hasSaturationChange) {
        ...
        if (hasBlur) {
            ...
            vImage_Error error = vImageBoxConvolve_ARGB8888(&effectInBuffer, &effectOutBuffer, NULL, (long)0, (long)0, (int)radius, (int)radius, 0, kvImageEdgeExtend);
